...
void draw() {
  ...
  byte digits = 8;    // max of 32 (32 bit standard integer)

  /* Calculate */
  byte input = 64;
  byte secondInput = (byte)2;
  byte output = (byte)(input >> secondInput);
...
void drawBinary(byte theValue, int theDigits, float theY) {
  pushMatrix();
